...
builder.Services.ConfigureApplicationCookie(o=>o.Events.OnValidatePrincipal=async c=>{var um=c.HttpContext.RequestServices.GetRequiredService<UserManager<ApplicationUser>>();var id=um.GetUserId(c.Principal!);if(id is null)return;var u=await um.FindByIdAsync(id);if(u is null||!u.IsActive){c.RejectPrincipal();}}); builder.Services.AddAuthorization(o=>{foreach(var r in Roles.All)o.AddPolicy(r,p=>p.RequireRole(r));}); builder.Services.AddScoped<DemoSeeder>();builder.Services.AddScoped<RequestWorkflowService>();builder.Services.AddScoped<ScopeService>();builder.Services.AddRazorPages();
var app=builder.Build(); if(app.Environment.IsDevelopment()){app.UseMigrationsEndPoint();using var scope=app.Services.CreateScope();await scope.ServiceProvider.GetRequiredService<DemoSeeder>().SeedAsync();}else{app.UseExceptionHandler("/Error");app.UseHsts();} app.UseHttpsRedirection();app.UseRouting();app.UseAuthentication();app.UseAuthorization();app.UseAntiforgery();app.MapStaticAssets();app.MapRazorPages().WithStaticAssets();app.Run();
