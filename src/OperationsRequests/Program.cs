...
builder.Services.ConfigureApplicationCookie(o=>o.Events.OnValidatePrincipal=async c=>{var um=c.HttpContext.RequestServices.GetRequiredService<UserManager<ApplicationUser>>();var u=await um.GetUserAsync(c.Principal!);if(u is null||!u.IsActive){c.RejectPrincipal();}}); builder.Services.AddAuthorization(o=>{foreach(var r in Roles.All)o.AddPolicy(r,p=>p.RequireRole(r));}); builder.Services.AddScoped<DemoSeeder>();builder.Services.AddScoped<RequestWorkflowService>();builder.Services.AddRazorPages();
